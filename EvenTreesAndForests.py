# Лес из четных деревьев. Получение из дерева (общий случай). Ф-я EvenTrees() возвращает максимально возможный список
# парных узлов SimpleTreeNode, между которыми для получения леса четных деревьев нужно разорвать связь
